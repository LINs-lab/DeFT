...
@triton.jit
def _fwd_kernel_stage1(  # type: ignore
    Q,
    K_Buffer,
    sm_scale,
    # Req_to_tokens,
    ...
    for start_mark in range(0, block_mask, 1):
        ...
        offs_buf_k = (
            cur_batch * stride_buf_kb
            + offs_n_new[:, None] * stride_buf_ks
        ...
        off_o = cur_head * att_stride_h + (
            cur_batch_in_all_start_index + offs_n
        )
        tl.store(
            Att_Out + off_o, att_value, mask=offs_n_new < cur_batch_end_index
        )


@triton.jit
def _fwd_kernel_stage2(  # type: ignore
    Logics,
    V_Buffer,
    Out,
    Req_to_tokens,
...
cached_kernel_stage2: Any = None


def _token_att_m_fwd(  # type: ignore
    q,
    k_buffer,
    att_out,
    Req_to_tokens,
    ...
    torch.cuda.nvtx.range_push("token_fwd_stage1")
...
def _token_softmax_reducev_fwd(  # type: ignore
    logics,
    v_buffer,
    o,
    req_to_tokens,
    ...
    torch.cuda.nvtx.range_push("token_fwd_stage2")
...
def token_attention_fwd(  # type: ignore
    q,
    k_buffer,
    v_buffer,
    o,
