...
class LogitsProcessor(nn.Module):
    ...
    def forward(self, input_ids, hidden_states, weight, input_metadata):  # type: ignore
        last_index = None
        is_decode = input_metadata.forward_mode != ForwardMode.PREFILL
        ...
        if not input_metadata.return_logprob:
            ...
            all_logprobs = torch.log(
                torch.softmax(logits.float(), dim=-1) + 1e-6
            )
            ...
            if is_decode:
                ...
                prefill_logprobs = all_logprobs[
                    ...
                    torch.cat(
                        [input_ids[1:], torch.tensor([0], device="cuda")]
                    ),
                ]
